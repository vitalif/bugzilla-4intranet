...
sub create {
    ...
    # Create product
    my $args = {};
    foreach my $field (Bugzilla::Product->DB_COLUMNS, qw(version classification)) {
        my $name = MAPPED_RETURNS->{$field} || $field;
        if (defined $params->{$name}) {
            $args->{$field} = $params->{$name};
        }
    }
    my $product = Bugzilla::Product->create($args);
    if ($params->{create_series})
    {
        $product->_create_series;
    }
...
sub _product_to_hash {
    ...
    my $usecl = Bugzilla->get_field('classification')->enabled;
    my $field_data = {
        id              => $self->type('int', $product->id),
        name            => $self->type('string', $product->name),
        description     => $self->type('string', $product->description),
        entryheaderhtml => $self->type('string', $product->entryheaderhtml),
        wiki_url        => $self->type('string', $product->wiki_url),
        is_active       => $self->type('boolean', $product->isactive),
        notimetracking  => $self->type('boolean', $product->notimetracking),
        has_unconfirmed => $self->type('boolean', $product->allows_unconfirmed),
        classification  => $self->type('string', $usecl ? $product->classification_obj->name : ''),
        extproduct      => $self->type('string', $product->extproduct && $product->extproduct->name),
        cc_group        => $self->type('string', $product->cc_group && $product->cc_group->name),
        votesperuser    => $self->type('int', $product->votesperuser),
        maxvotesperbug  => $self->type('int', $product->maxvotesperbug),
        votestoconfirm  => $self->type('int', $product->votestoconfirm),
    };
...
sub _flag_type_to_hash {
    ...
    return filter $params, {
        ...
        cc_list =>
            $self->type('string', $flag_type->cc_list_obj),
        sort_key =>
