...
sub fields {
    ...
    foreach my $field (@fields) {
        ...
        $vis_values = $field->visibility_field ? $field->visibility_field->value_type->new_from_list($vis_values) : [];
...
sub get {
    ...
    # Do map {} instead of grep {} to untaint these IDs
    my @int = map { /^(\d+)$/so } @$ids;
...
sub _bug_to_hash {
    ...
    foreach my $field (@custom_fields) {
        ...
        elsif ($field->type == FIELD_TYPE_DATETIME) # FIXME: || $field->type == FIELD_TYPE_DATE
        {
