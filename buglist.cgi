...
# CustIS Bug 68921 - "—упер-TodayWorktime", или массова€ фиксаци€ трудозатрат
# по нескольким багам, за нескольких сотрудников, за различные периоды.
# ƒл€ фиксации времени задним числом / другим юзером требует группу worktimeadmin.

# FIXME сейчас оно встроено сюда, ибо большј€ часть логики, отвечающей за
#   создание и выполнение запроса поиска, находитс€ не в Bugzilla::Search, а именно здесь :-(
#   кусочки логики наход€тс€ здесь по слову superworktime.
my $superworktime;
...
{
    # ѕара хаков:
    # во-первых, _utf8_off не работает на бывшем когда-то tainted скал€ре,
    #   а from_to не работает на скал€ре с включЄнным utf8 флагом, поэтому мы
    #   его копируем и работаем с копией.
    # во-вторых, заголовки вывод€тс€ с включЄнным utf8 флагом, а мы не хотим,
    #   чтобы perl сам что-то перекодировал - поэтому в конце идЄт _utf8_on.
    trick_taint($output);
    my $untaint = $output;
