...
/*        // If the header contains a link, clear the href.
    ...
    _check_already_sorted: function(cell) {
        if (cell.className.search(/\bsorted_0\b/) == -1)
            return 0;
...
/*    sorttable._remove_sorting_marker();
    ...
    sort_table: function(cell) {
        ...
        for (var j=0; j<rows.length; j++) {
            if (j % 2)
                rows[j].className = rows[j].className.replace('bz_row_even', 'bz_row_odd');
            else
                rows[j].className = rows[j].className.replace('bz_row_odd', 'bz_row_even');
    ...
    reverse_table: function(cell) {
        ...
        for (var i = newrows.length-1; i >= 0; i--) {
            if (i % 2)
                newrows[i].className = newrows[i].className.replace('bz_row_even', 'bz_row_odd');
            else
                newrows[i].className = newrows[i].className.replace('bz_row_odd', 'bz_row_even');
